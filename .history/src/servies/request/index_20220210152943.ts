...
class YcRequest {
  ...
  get<T>(config: YcResquestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: "GET" });
  }

  post<T>(config: YcResquestConfig<T>): Promise<T> {
    console.log(config.baseURL);

    return this.request<T>({ ...config, method: "POST" });
  }

  delete<T>(config: YcResquestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: "DELETE" });
  }

  patch<T>(config: YcResquestConfig<T>): Promise<T> {
